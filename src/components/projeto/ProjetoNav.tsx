  ...
  return (
    <header className="sticky top-0 z-40 bg-pj-bg/90 backdrop-blur">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-4 px-5 py-4 sm:px-10">
        ...
        <nav className="flex items-center gap-1.5 rounded-full border border-pj-line bg-pj-paper p-1.5">
          {LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="rounded-full px-3.5 py-2 text-[13px] font-medium transition hover:bg-pj-bg2 sm:px-4"
            >
              {l.label}
            </a>
          ))}
